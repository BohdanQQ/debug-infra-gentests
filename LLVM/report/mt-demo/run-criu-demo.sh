...
selection="skip"
binary="$here/mt-demo"
criuResults="$here/result/outputs/"
mkdir -p "$criuResults"

llcap="../../sandbox/02-ipc/llcap-server/target/debug/llcap-server"

$llcap  --modmap "$modmaps" --cleanup test

$llcap -vvvv --modmap "$modmaps" capture-args -s "$selection" -o "$capture" "$binary" 1 2 2

$llcap -vvvv --modmap "$modmaps" test --mode criu -s "$selection" -o "$criuResults" -c "$capture" "$binary" 1 2 2
