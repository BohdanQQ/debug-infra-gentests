...
const CLI_MSG_SIZE: usize = 16;

impl TryFrom<&[u8; CLI_MSG_SIZE]> for TestMessage {
  type Error = String;
  ...
  fn try_from(value: &[u8; CLI_MSG_SIZE]) -> Result<Self, Self::Error> {
    let (tag, data) = value.split_at(2);
...
pub struct MultithreadTestJobParams {
  // we run only a single test case as the entire process
  pub fn_uid: NumFunUid,
  pub test_index: u32,
...
async fn singular_test_job(
  ...
  let result = wait_or_terminate(test, job_params, fn_uid, call_idx).await;
...
async fn multithread_test_job(
  ...
  let call_idx = job_params.test_index;
  let uid = job_params.fn_uid;
  let cloned_params = job_params.clone();
  singular_test_job(
    metadata_svr.clone(),
...
async fn wait_or_terminate(
  ...
  if let Some(timeout_duration) = job_timeout {
    ...
    let _ = test.wait().await.inspect_err(|e| {
      lg.crit(format!(
        "Test wait {id:?}@{call_idx:?} failed with error {e}"
      ))
    });
    Ok(())
  }
